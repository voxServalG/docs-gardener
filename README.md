...
| `garden-polish` | 按浅白写作指导生成文档润色建议，不直接修改文件 |
| `garden-apply` | 批量编辑文档并创建 GitHub PR，失败自动回滚 |

`garden-polish` 的可应用建议固定使用 `file`、`oldText`、`newText`、`reason` 四个字段。工具结果仍包含 agent-facing envelope，例如 `display`、`hint`、`requires_user`、`stop_here` 和 `allowedTools`。
