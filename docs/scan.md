...
`garden-scan` 是组合入口。它先跑 `garden-scan-hard`，再跑 `garden-scan-soft`，返回一个组合 envelope。
...
- 组合 envelope 携带 `data.agentDirective`：这是 **硬环节**，调用方 agent 必须以硬代码按 `renderSchema` 处理，逐字向用户汇报，不得压缩或改写。
...
- `phase` = `scan`
- `next` = `garden-fix`
- `data.hard` / `data.soft`：完整子 envelope。
- `data.agentDirective`：`{ renderRequired, processingContract, renderSchema, acknowledge }`。
