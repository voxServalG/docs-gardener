...
The public package is `@voxserval/docs-gardener`; its stable bin is `docs-gardener`. The package publishes the runtime JavaScript under `src/` directly, so it has no compiled output or `dist/` directory. Registry installation does not run `prepare`, `install`, or `postinstall`.
...
npm trust github @voxserval/docs-gardener --file npm-publish.yml --repo voxServalG/docs-gardener --allow-publish
```
